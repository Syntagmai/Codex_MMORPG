            ...
            <div style="display: flex; gap: 20px; align-items: center;">
                <div style="flex: 1;">
                    ...
                    <div class="progress-bar">
                        <div class="progress-fill" style="width: {self._calculate_percentage(feedback_stats.get('sentiment_distribution', {}).get('positive', 0), feedback_stats.get('total_feedback', 1))}%"></div>
                    </div>
                </div>
                <div style="flex: 1;">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 5px;">
                    ...
                    <div class="progress-bar">
                        <div class="progress-fill" style="width: {self._calculate_percentage(feedback_stats.get('sentiment_distribution', {}).get('neutral', 0), feedback_stats.get('total_feedback', 1))}%"></div>
                    </div>
                </div>
                <div style="flex: 1;">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 5px;">
                    ...
                    <div class="progress-bar">
                        <div class="progress-fill" style="width: {self._calculate_percentage(feedback_stats.get('sentiment_distribution', {}).get('negative', 0), feedback_stats.get('total_feedback', 1))}%"></div>
                    </div>
                </div>
            </div>
        </div>
...
O sistema de auto aprendizado BMAD demonstrou **{'excelente' if learning_stats.get('patterns_learned', 0) > 20 else 'bom' if learning_stats.get('patterns_learned', 0) > 10 else 'moderado'}** desempenho no período analisado.
...
- **Positivo**: {feedback_stats.get('sentiment_distribution', {}).get('positive', 0)} ({self._calculate_percentage(feedback_stats.get('sentiment_distribution', {}).get('positive', 0), feedback_stats.get('total_feedback', 1)):.1f}%)
- **Neutro**: {feedback_stats.get('sentiment_distribution', {}).get('neutral', 0)} ({self._calculate_percentage(feedback_stats.get('sentiment_distribution', {}).get('neutral', 0), feedback_stats.get('total_feedback', 1)):.1f}%)
- **Negativo**: {feedback_stats.get('sentiment_distribution', {}).get('negative', 0)} ({self._calculate_percentage(feedback_stats.get('sentiment_distribution', {}).get('negative', 0), feedback_stats.get('total_feedback', 1)):.1f}%)
...
"""
    ...
    def _format_most_effective_rules(self, optimization_stats: Dict[str, Any]) -> str:
        ...
        for i, rule in enumerate(most_used_rules[:5], 1):
            formatted += f"{i}. **{rule['rule_type']}** - Uso: {rule['usage_count']}, Sucesso: {rule['success_rate']:.2f}\n"
