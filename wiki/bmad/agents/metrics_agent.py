...
class MetricsAgent:
    ...
    def collect_application_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"✅ Métricas da aplicação coletadas: {total_files} arquivos, {active_agents} agentes ativos")
            return metrics
...
- **Sistema**: {'🟢 Saudável' if kpis.get('overall_score', 0) >= 80 else '🟡 Atenção' if kpis.get('overall_score', 0) >= 60 else '🔴 Crítico'}
