...
from module_analyzer import ModuleAnalyzerAgent
from module_generator import ModuleGeneratorAgent
...
class WorkflowOrchestrator:
    ...
    def execute_generation_phase(self, analysis_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            generation_results = {
                ...
                "average_compatibility_score": sum(v.get("compatibility_score", 0) for v in variations) / len(variations) if variations else 0,
                "status": "success"
            }
    ...
    def generate_workflow_summary(self, workflow_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "generation" in workflow_results["phases"]:
            ...
            summary["key_metrics"]["success_rate"] = gen_results.get("successful_variations", 0) / max(gen_results.get("total_variations", 1), 1)
        ...
        if "testing" in workflow_results["phases"]:
            ...
            summary["key_metrics"]["test_pass_rate"] = test_summary.get("passed_variations", 0) / max(test_summary.get("total_variations", 1), 1)
